...
SPLIT=json.load(open("analyses/material_split.json"))["split"]; F={"steel":1.8,"concrete":0.12,"plastic":3.0}
...
def rows_html(view):
    ...
    for title,n in GROUPS:
        ...
        if view=="mass": out+=f'<div class="hb hd"><span class="hl">{title}</span><div class="tr">{mass_bar(gk,gsp)}</div><span class="hv"><b>{gk*1000:,.0f} tons</b></span></div>\n'
        else:
            gparts=[(m,sum(kt*SPLIT[nm][j]*F[m] for nm,kt in grp)) for j,m in enumerate(("steel","concrete","plastic"))]; gt=sum(v for _,v in gparts)
            out+=f'<div class="hb hd"><span class="hl">{title}</span><div class="tr">{carb_bar(gparts)}</div><span class="hv"><b>{gt*1000:,.0f} tons CO₂e</b></span></div>\n'
        for name,kt in grp:
            sp=SPLIT[name]
            if view=="mass": out+=f'<div class="hb"><span class="hl">{name}</span><div class="tr">{mass_bar(kt,sp)}</div><span class="hv">{kt*1000:,.0f} tons</span></div>\n'
            else:
                parts=carb_parts(kt,sp); tot=sum(v for _,v in parts); tag=" <em>other materials not valued</em>" if sp[3]>=0.3 else ""
                out+=f'<div class="hb"><span class="hl">{name}</span><div class="tr">{carb_bar(parts)}</div><span class="hv">{tot*1000:,.0f} tons CO₂e{tag}</span></div>\n'
        i+=n
    return out
AX_M='<div class="hb ax"><span class="hl"></span><div class="tr ticks"><span style="left:8.0%">1</span><span style="left:34.4%">10</span><span style="left:60.9%">100</span><span style="left:87.4%">1,000</span></div><span class="hv">\'000s of tons, log scale</span></div>'
AX_C='<div class="hb ax"><span class="hl"></span><div class="tr ticks"><span style="left:0%">0</span><span style="left:20%">100</span><span style="left:40%">200</span><span style="left:60%">300</span><span style="left:80%">400</span><span style="left:100%">500</span></div><span class="hv">\'000s of tons CO₂e</span></div>'
