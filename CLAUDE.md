...
- `api/enter.mjs` — validates the email shape, records the signup to Vercel Blob (`deck-signups/` prefix in the `deck-signups` store), sets an HMAC-signed cookie (secret: `AUTH_SECRET` env var, 30-day expiry), and redirects to `/intro?v=<email>` so the deck's per-viewer analytics attribute automatically.
