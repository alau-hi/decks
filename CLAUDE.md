...
- `gate.html` — the one branded gate form (middleware rewrites to it, URL preserved); it asks for the email, the deck's shared password, or both, depending on what the visitor still owes for the page they asked for. No site-wide access code (one existed originally; the unused `DECK_PASSWORD` env var remains if it's ever wanted back).
