...
- **`vercel.json`:** add `redirects: [{ source: "/", destination: "/intro", permanent: false }]` and `rewrites: [{ source: "/intro", destination: "/" }]`. The rewrite destination is `/`, not `/index.html`: with `cleanUrls` enabled Vercel redirects `/index.html` back to `/`, so rewriting to the file path 404s (verified live during implementation). Rewrites are internal — the `/` destination serves `index.html` directly and does not re-enter the `/` → `/intro` redirect. 302 (not permanent) so browsers never cache the redirect irrevocably. Query strings pass through automatically — existing `/?v=<email>` links land on `/intro?v=<email>`.
- **`middleware.js`:** two changes. (1) A root 302 to `/intro` (query preserved) ahead of any auth handling, as defense-in-depth: in practice `vercel.json`'s non-permanent redirect fires first (observed 307 in production) and this branch stays dormant, but it keeps gated behavior independent of Vercel's routing order. (2) The viewer-identity redirect (injects `?v=<email>` for cookied visitors) matches `path === '/intro'` instead of `path === '/'`. Flow for a cookied root visit: middleware passes `/` through → Vercel redirects to `/intro` → middleware stamps `?v=` → rewrite serves the deck. Unauthenticated `/intro` visits hit the existing gate rewrite with the URL preserved (gate shows at `/intro`). No other auth logic changes.
...
- `GET /` → 307/302 (temporary) to `/intro` (query preserved) on gated and ungated deployments.
